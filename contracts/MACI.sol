...
contract MACI is DomainObjs, SnarkCommon, Ownable {
    ...
    function signUp(PubKey memory _pubKey, bytes memory _data)
        public
        atPeriod(Period.Voting)
    {
        ...
        uint256 stateLeaf = hashStateLeaf(StateLeaf(_pubKey, balance, 0, 0));
    ...
    function stopVotingPeriod(uint256 _maxVoteOptions)
        public
        onlyOwner
        atPeriod(Period.Voting)
    {
    ...
    function processMessage(uint256 newStateCommitment, bytes memory _proof)
        public
        atPeriod(Period.Processing)
    {
        require(
        ...
        bytes memory vk = vkRegistry.getProcessVk(
    ...
    function processTally(uint256 newTallyCommitment, bytes memory _proof)
        public
        atPeriod(Period.Tallying)
    {
        ...
        bytes memory vk = vkRegistry.getTallyVk(
            parameters.stateTreeDepth,
